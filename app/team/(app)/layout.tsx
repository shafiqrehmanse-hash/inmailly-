...
export default async function TeamAppLayout({
  ...
  const [{ data: teamLeaders }, { count }] = await Promise.all([
    admin
      .from("team_members")
  ...
  const chatMode = canOpenLiveChat(member.role)
    ? ("member" as const)
    : isTeamLeader(member.role) && member.live_chat_agent
      ? ("leader" as const)
      : null;
  ...
  return (
    <div className="min-h-screen bg-lux-bg text-lux-text relative">
      <WorkspaceAmbient />
      <Sidebar member={member} poolCount={count || 0} teamLeaders={teamLeaders || []} />
